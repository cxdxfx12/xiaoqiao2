...
> **🚨 2026-07-24 下午新增两项大改动（见 PROJECT_v1.1_new.md 为最新完整文档）**：
> - **功能7（T7）快递模板自动识别 + 可视化管理**：批量计算页新增「⚙️ 管理模板」入口 → 总开关 + 8家快递逐条启用禁用 + 双击行可点开编辑 → 自定义模板同ID自动覆盖内置，config.ini 持久化；
> - **项目图标全套升级为「祥云快递盒」方案B**：Mac App Bundle（.icns 392KB）/Dock/Finder/所有窗口标题栏/Alt-Tab/关于页 Logo/QRC 9档PNG(16→1024px) 处处一致，新增 gen_icon 工具一键再生成。
