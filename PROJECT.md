...
> **🚨 最新改动 2026-07-27 v1.09 已同步至 PROJECT_v1.1_new.md（本项目最完整最新版）**
> 本次大版本修复拉均重（平均重量合同定价）真实场景 6 坑 + 打包部署版本号管理规范化：
> 1. **拉均重真实场景端到端打通（订单CSV填中文客户名也能匹配到 / 方案A分区不被多余 template_id 过滤误杀）** — PROJECT_v1.1_new.md §9.7 坑 20-24
> 2. **CMake VERSION 为唯一真源** — 改一处自动传播到 Info.plist / 关于页显示 / 诊断列 / 日志；PROJECT_v1.1_new.md §10.6 有完整 v1.00~v1.09 发布历史
> 3. **billing_params_test T7 / T7.1 / T8 三条用例长期回归守护**（方案A / 方案B / 模板级绑定 / 排除省 / 真实 CSV 工作流）
> 4. 其他大改动请继续跳转 PROJECT_v1.1_new.md 对应章节：快递模板自动识别 + 可视化管理、祥云快递盒图标全套 v1.05，授权系统 6 坑修复 v1.06，客户级覆写 + 7 标准列 + 4 条全局加价策略 v1.02~v1.04
